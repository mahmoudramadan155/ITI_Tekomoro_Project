...
class Model(object):
  ...
  def __init__(self, config, scope):
    ...
    self.obs_length = tf.compat.v1.placeholder(
        "int32", [N], name="obs_length")
    self.pred_length = tf.compat.v1.placeholder(
        "int32", [N], name="obs_length")
    ...
    self.obs_scene = tf.compat.v1.placeholder("int32", [N, None], name="obs_scene")
    self.obs_scene_mask = tf.compat.v1.placeholder(
        "bool", [N, None], name="obs_scene_mask")
    # the actual feature
    self.scene_feat = tf.compat.v1.placeholder(
    ...
    for h, w in config.scene_grids:
      ...
      self.grid_pred_labels.append(
          tf.compat.v1.placeholder("int32", [N]))  # grid class
      self.grid_pred_targets.append(tf.compat.v1.placeholder("float32", [N, 2]))

      self.grid_obs_labels.append(
          tf.compat.v1.placeholder("int32", [N, None]))  # grid class
      self.grid_obs_targets.append(
          tf.compat.v1.placeholder("float32", [N, None, 2]))


      self.grid_obs_regress.append(
          tf.compat.v1.placeholder("float32", [N, None, h, w, 2],
                         name="grid_obs_regress"))
      if config.use_soft_grid_class:
        self.grid_pred_labels_T.append(
            tf.compat.v1.placeholder("float32", [N, None, h, w, 1], name="grid_pred_classes"))
      else:
        self.grid_pred_labels_T.append(
            tf.compat.v1.placeholder("float32", [N, None], name="grid_pred_classes"))
  ...
  def build_forward(self):
    ...
    with tf.compat.v1.variable_scope("person_pred") as top_scope:
      ...
      for i, (h, w) in enumerate(config.scene_grids):
        ...
        enc_cell_obs_grid = tf.keras.layers.ConvLSTM2D(
            2, input_shape=grid_rnn_input_shape,
            # output_channels=config.enc_hidden_size,
            kernel_size=[config.convlstm_kernel, config.convlstm_kernel],
            name="enc_grid_%d" % i)
        enc_cell_obs_grid = tf.keras.layers.Dropout(
            enc_cell_obs_grid, keep_prob)
        enc_cell_obs_grid_reg = tf.keras.layers.ConvLSTM2D(
            2, input_shape=[h, w, 2],
            # output_channels=config.enc_hidden_size,
            kernel_size=[config.convlstm_kernel, config.convlstm_kernel],
            name="enc_grid_regress_%d" % i)
        enc_cell_obs_grid_reg = tf.keras.layers.Dropout(
        ...
        dec_cell_grid = tf.keras.layers.ConvLSTM2D(
            2, input_shape=[h, w, config.enc_hidden_size],
            # output_channels=config.dec_hidden_size,
            kernel_size=[config.convlstm_kernel, config.convlstm_kernel],
            name="dec_grid_%d" % i)
        dec_cell_grid = tf.keras.layers.Dropout(
            dec_cell_grid, keep_prob)
        dec_cell_grid_reg = tf.keras.layers.ConvLSTM2D(
            2, input_shape=[h, w, config.enc_hidden_size],
            # output_channels=config.dec_hidden_size,
            kernel_size=[config.convlstm_kernel, config.convlstm_kernel],
            name="dec_grid_reg_%d" % i)
        dec_cell_grid_reg = tf.keras.layers.Dropout(
...
def conv2d(x, out_channel, kernel, padding="SAME", stride=1,
  ...
  with tf.compat.v1.variable_scope(scope, reuse=tf.compat.v1.AUTO_REUSE):
    ...
    if w_init is None:
      w_init = tf.compat.v1.variance_scaling_initializer(scale=2.0)
